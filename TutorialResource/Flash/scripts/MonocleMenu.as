package
{
   ...
   public class MonocleMenu extends IMenu
   {
      ...
      public var Payload_mc:MovieClip;
      
      public var Storage_mc:MovieClip;
      
      public var CurrentXP_mc:MovieClip;
      
      public var NextLvlXP_mc:MovieClip;
      ...
      private var Payload_OrigVert:Number = 0;
      
      private var Payload_OrigHort:Number = 0;
      
      private var Storage_OrigVert:Number = 0;
      
      private var Storage_OrigHort:Number = 0;
      
      private var CurrentXP_OrigVert:Number = 0;
      
      private var CurrentXP_OrigHort:Number = 0;
      
      private var NextLvlXP_OrigVert:Number = 0;
      
      private var NextLvlXP_OrigHort:Number = 0;
      
      private var Time_OrigVert:Number = 0;
      
      private var Time_OrigHort:Number = 0;
      ...
      public function MonocleMenu()
      {
         ...
         super();
         addFrameScript(0,this.frame1,8,this.frame9,15,this.frame16,16,this.frame17,27,this.frame28);
         ...
         BSUIDataManager.Subscribe("PlayerInventoryData",this.onPlayerInventoryUpdate);
      ...
      {
         var encumbrance:uint = uint(param1.data.fEncumbrance);
         var maxEncumbrance:uint = uint(param1.data.fMaxEncumbrance);
         var itemCoins:uint = uint(param1.data.uCoin);
         if(configIni.Encumbrance_bMassLeftToRight == 0)
         {
            GlobalFunc.SetText(Payload_mc.Payload_tf,configIni.Encumbrance_sMassLabel + maxEncumbrance + "/");
            GlobalFunc.SetText(Storage_mc.Storage_tf,encumbrance + configIni.Encumbrance_sMetricMass + "(" + itemCoins + configIni.Encumbrance_sCreditLabel + ")");
         }
         else
         {
            GlobalFunc.SetText(Payload_mc.Payload_tf,configIni.Encumbrance_sMassLabel + encumbrance + "/");
            GlobalFunc.SetText(Storage_mc.Storage_tf,maxEncumbrance + configIni.Encumbrance_sMetricMass + "(" + itemCoins + configIni.Encumbrance_sCreditLabel + ")");
         }
         var invColorCondition:int = int(ColorTransformUI.GetPayloadColorCondition(encumbrance,maxEncumbrance));
         var invColorPayload:ColorTransform = new ColorTransform();
         var invColorStorage:ColorTransform = new ColorTransform();
         switch(invColorCondition)
         {
            case 0:
               invColorPayload.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColorMax);
               invColorStorage.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColor);
               break;
            case 1:
               invColorPayload.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColorMax);
               invColorStorage.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColor80);
               break;
            case 2:
               invColorPayload.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColorMax);
               invColorStorage.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColor90);
               break;
            case 3:
               invColorPayload.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColorMax);
               invColorStorage.color = ColorTransformUI.convertColor(configIni.Encumbrance_sEncTextColor100);
         }
         if(configIni.Encumbrance_bMassLeftToRight == 0)
         {
            Payload_mc.transform.colorTransform = invColorPayload;
            Storage_mc.transform.colorTransform = invColorStorage;
         }
         else
         {
            Payload_mc.transform.colorTransform = invColorStorage;
            Storage_mc.transform.colorTransform = invColorPayload;
         }
      }
      
      private function onPlayerDataUpdate(param1:FromClientDataEvent) : void
      {
         var currentXP:uint = uint(param1.data.fLevelXP);
         var nextLvlXP:uint = uint(param1.data.fNextLevelXP);
         var currentLvl:uint = uint(param1.data.uLevel);
         var skillPoints:uint = uint(param1.data.uSkillPoints);
         var currentColorXP:ColorTransform = new ColorTransform();
         var nextColorXP:ColorTransform = new ColorTransform();
         currentColorXP.color = ColorTransformUI.convertColor(configIni.Experience_sExpCurTextColor);
         nextColorXP.color = ColorTransformUI.convertColor(configIni.Experience_sExpNxtTextColor);
         GlobalFunc.SetText(CurrentXP_mc.CurrentXP_tf,configIni.Experience_sLevelLabel + "(" + currentLvl + "):" + nextLvlXP + "/");
         GlobalFunc.SetText(NextLvlXP_mc.NextLvlXP_tf,currentXP + "(" + skillPoints + ")");
         CurrentXP_mc.transform.colorTransform = currentColorXP;
         NextLvlXP_mc.transform.colorTransform = nextColorXP;
      }
      
      private function UpdateTime() : *
      {
         var timeColor:ColorTransform = new ColorTransform();
         var globalTime:uint = Math.floor(this.GalacticStandardTime);
         var globalMinute:uint = Math.floor((this.GalacticStandardTime - globalTime) * 60);
         var localTime:Number = NaN;
         var localHour:uint = 0;
         var localMinute:uint = 0;
         if(this.InSpaceship || this.LocalPlanetHoursPerDay <= 0)
         {
            GlobalFunc.SetText(Time_mc.Time_tf,"");
         }
         else
         {
            localTime = this.LocalPlanetTime * 24;
            localHour = Math.floor(localTime);
            localMinute = Math.floor((localTime - localHour) * 60);
            GlobalFunc.SetText(Time_mc.Time_tf,configIni.Time_sTimeLabel + "(lt/gt):" + GlobalFunc.PadNumber(localHour,2) + ":" + GlobalFunc.PadNumber(localMinute,2) + "/" + GlobalFunc.PadNumber(globalTime,2) + ":" + GlobalFunc.PadNumber(globalMinute,2));
         }
         timeColor.color = ColorTransformUI.convertColor(configIni.Time_sTimeTextColor);
         Time_mc.transform.colorTransform = timeColor;
      }
      ...
      private function onPickDataUpdate(param1:FromClientDataEvent) : void
      {
         ...
         this.UpdateButtonData();
         this.Payload_mc.y = this.Payload_OrigHort + configIni.Encumbrance_fEncCurrentMassY;
         this.Payload_mc.x = this.Payload_OrigVert + configIni.Encumbrance_fEncCurrentMassX;
         this.Storage_mc.y = this.Storage_OrigHort + configIni.Encumbrance_fEncMaxMassY;
         this.Storage_mc.x = this.Storage_OrigVert + configIni.Encumbrance_fEncMaxMassX;
         this.CurrentXP_mc.y = this.CurrentXP_OrigHort + configIni.Experience_fExpCurrentExpY;
         this.CurrentXP_mc.x = this.CurrentXP_OrigVert + configIni.Experience_fExpCurrentExpX;
         this.NextLvlXP_mc.y = this.NextLvlXP_OrigHort + configIni.Experience_fExpNextExpY;
         this.NextLvlXP_mc.x = this.NextLvlXP_OrigVert + configIni.Experience_fExpNextExpX;
         this.Time_mc.y = this.Time_OrigHort + configIni.Time_fTimeHorY;
         this.Time_mc.x = this.Time_OrigVert + configIni.Time_fTimeVerX;
      }
      ...
      private function onMonocleDataUpdate(param1:FromClientDataEvent) : void
      {
         ...
         else
         {
            ...
            newFrame = String(getCurrFrame(arEvent.data.uFaunaCurrent,arEvent.data.uFaunaMax));
            if(newFrame != this.PlanetInfo_mc.Fauna_mc.currentFrameLabel)
            {
               this.PlanetInfo_mc.Fauna_mc.gotoAndStop(newFrame);
            }
            newFrame = String(getCurrFrame(arEvent.data.uFloraCurrent,arEvent.data.uFloraMax));
            if(newFrame != this.PlanetInfo_mc.Flora_mc.currentFrameLabel)
            {
               this.PlanetInfo_mc.Flora_mc.gotoAndStop(newFrame);
            }
            newFrame = String(getCurrFrame(arEvent.data.uResourcesCurrent,arEvent.data.uResourcesMax));
      ...
      {
         BSUIDataManager.dispatchEvent(new CustomEvent("MonocleMenu_Initialize",{"width":this.getChildByName("OuterCircle_mc").width}));
         dispatchEvent(new Event("onOpenAnimComplete"));
      }
      ...
      {
         BSUIDataManager.dispatchEvent(new CustomEvent("MonocleMenu_Shutdown",null));
      }
      
      internal function frame28() : *
      {
         stop();
         dispatchEvent(new Event("onCloseAnimComplete"));
      }
   }
}
